...
// 		}
	...
	public static function ReplaceVarInSql($sql) {
		...
		preg_match("/\\{\\$[\\S\\s]+?\\$\\}/",$sql,$_sqlArr);
	
		for ($_i = 0; $_i < count($_sqlArr); $_i++) {
			$_tmpStr = trim($_sqlArr[$_i]);
			...
			if (count($_strList) == 1) {
				$_retSql = str_replace($_sqlArr[$_i],Request::Get($_strList[0]),$_retSql);
			}
			else if (strtolower($_strList[0]) == "r") {
				$_retSql = str_replace($_sqlArr[$_i],Request::Get($_strList[1]),$_retSql);
			}
			else if (strtolower($_strList[0]) == "s") {
				$_retSql = str_replace($_sqlArr[$_i],Session::Get($_strList[1]),$_retSql);  
			}
			else if (strtolower($_strList[0]) == "c") {
				$_retSql = str_replace($_sqlArr[$_i],Cookies::Get($_strList[1]),$_retSql);
			}
		}
